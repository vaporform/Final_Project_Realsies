...
class Abigor(Demon):
    def __init__(self,deck,hand=[]):
        super().__init__("Arzolcam", "Commander of lower-demons. Likes to play it fair and square. He's too focused to speak.", deck, hand)
...
class Fafnir(Demon):
    def __init__(self,deck,hand=[]):
        super().__init__("Farina", "Dragon that can smell value and worth. It is blinded by numbers.", deck, hand)
    ...
    def decide(self, grid: Grid):
        ...
        if random.random() > (1.0 - self.aggression):
            ...
            skill = self.choose_helper()
            self.aggression = 0
            self.description = random.choice(["FARINA: X MARKS THE SPOT! FARINA CAN GREATLY SENSE IT! HUMAN GREEDY!", 
            "FARINA: FARINA OWNS THIS CARD!",
            "FARINA: FARINA HOLDS THIS CARD! HUMAN NOT PICKING!"])
            return "ACTION", skill
...
class Baphomet(Demon):
    def __init__(self, deck, hand=[]):
        super().__init__("Devil", "The King of Demons. Has tricks up his sleeves.", deck, hand)
    ...
    def decide(self, grid: Grid):
        ...
        if random.random() > (1.0 - self.aggression): # play helpers!
            ...
            match skill.name:
                case "Lock":
                    flavor_text = random.choice(["DEVIL: I shall withold you from picking that card.",
                    "DEVIL: I won't allow that to happen.",
                    "DEVIL: Less choices, hm?"])
                case "Bounty":
                    flavor_text = random.choice([
                        'DEVIL: Greedy always stays ahead.',
                        "DEVIL: I shall boost this card to my liking.",
                        "DEVIL: I can sense a strong card... Boosting."])
                case "Trap":
                    flavor_text = random.choice(['DEVIL: This trap shall do nicely...',
                    "DEVIL: Hmmm... I wonder will you pick that card?"])
                case "Curse":
                    flavor_text = random.choice(["DEVIL: Can you sense your impending doom, mortal?",
                    "DEVIL: Don't you feel a bit off, mortal?",
                    "DEVIL: You seem unwell, mortal."])
